...
struct ArrayTesterUtils
{
	...
	static void
	clone_c_array(xstl::array<_Ty, _Size> &_arr, _Ty *_c_arr)
	{
		size_t i = 0;
		for (auto &elem : _arr)
		{
	...
	template<typename _Ty, size_t _Size>
	static void
	init_xstl_array(xstl::array<_Ty, _Size> &_arr)
	{
		for (auto &_e : _arr)
...
protected:
	...
	_Array_tester(const xstl::array<_Ty, _Size> &arr, _Ty *ref) : _arr(arr), _ref()
	{
	...
	_test_forward_iteration()
	{
		...
		for (
			struct {
				size_t i;
				typename xstl::array<_Ty, _Size>::iterator _it;
			} loop = {0, _arr.begin()};
			loop._it != _arr.end();
			++loop._it, ++loop.i)
		{
		...
		for (
			struct {
				size_t i;
				typename xstl::array<_Ty, _Size>::const_iterator _it;
			} loop = {0, _carr.begin()};
			loop._it != _carr.end();
			++loop._it, ++loop.i)
		{
	...
	_test_reverse_iteration()
	{
		...
		for (
			struct {
				size_t i;
				typename xstl::array<_Ty, _Size>::reverse_iterator _it;
			} loop = {_Size - 1, _arr.rbegin()};
			loop._it != _arr.rend();
			++loop._it, --loop.i)
		{
		...
		for (
			struct {
				size_t i;
				typename xstl::array<_Ty, _Size>::const_reverse_iterator _it;
			} loop = {_Size - 1, _carr.crbegin()};
			loop._it != _carr.rend();
			++loop._it, --loop.i)
		{
	...
	_test_generated_c_array()
	{
		...
		_array_assert(_arr_data, _ref, _Size, [](_Ty x, _Ty y) { _xstl_assert(x == y, "data() failed"); });
		_array_assert(_carr_data, _ref, _Size, [](_Ty x, _Ty y) { _xstl_assert(x == y, "data() const overload failed"); });
	}
	...
	_test_fill()
	{
		...
		_array_assert(_arr, _new_arr, _Size, [](_Ty x, _Ty y) { _xstl_assert(x == y, "fill(const_reference) failed"); });
	}
...
// constexpr if statement is not supported in __cplusplus < 201703L
	...
	_test_print() const
	{
		...
		{
			_expected = "{" + std::to_string(_val) + ", " + std::to_string(_val) + ", " + std::to_string(_val) + ", " +
						std::to_string(_val) + ", " + std::to_string(_val) + "}";
		}
	...
	_test_print()
	{
		...
		std::string _expected = "";
		_expected = "{" + std::to_string(_val) + ", " + std::to_string(_val) + ", " + std::to_string(_val) + ", " + std::to_string(_val) +
					", " + std::to_string(_val) + "}";
...
private:
	...
	void
	_array_assert(_Arr1 _arr1, _Arr2 _arr2, std::size_t _N, Assertion assert_function) const
	{
...
struct ArrayTester : protected _Array_tester<_Ty, _Size>
{
	...
	test_print();
	void
	perform_all_initial_checks();
};

template<typename _Ty, std::size_t _Size>
void
ArrayTester<_Ty, _Size>::perform_storage_checks()
{
	this->_test_size();
	this->_test_max_size();
	this->_test_empty();
...
template<typename _Ty, std::size_t _Size>
void
ArrayTester<_Ty, _Size>::perform_accessors_checks()
{
	this->_test_access_operator();
	this->_test_at();
	this->_test_front();
...
template<typename _Ty, std::size_t _Size>
void
ArrayTester<_Ty, _Size>::perform_iterator_checks()
{
...
template<typename _Ty, std::size_t _Size>
void
ArrayTester<_Ty, _Size>::perform_modifiers_checks()
{
	this->_test_fill();
	this->_test_swap();
}
...
template<typename _Ty, std::size_t _Size>
void
ArrayTester<_Ty, _Size>::perform_relational_ops_checks()
{
...
template<typename _Ty, std::size_t _Size>
void
ArrayTester<_Ty, _Size>::test_print()
{
...
template<typename _Ty, std::size_t _Size>
void
ArrayTester<_Ty, _Size>::perform_all_initial_checks()
{
	perform_iterator_checks();
	perform_storage_checks();
	perform_accessors_checks();
